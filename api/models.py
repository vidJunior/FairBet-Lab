...
from config.choices import (
    TipoAccionAuditoria,
    ReglaActividadSospechosa,
    EstadoActividadSospechosa,
    SeveridadActividadSospechosa,
)


class AuditLog(models.Model):
    timestamp = models.DateTimeField(auto_now_add=True)
    action_type = models.CharField(max_length=20, choices=TipoAccionAuditoria.choices)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            raise ValidationError(
                "Los registros de auditoría son inmutables y no se pueden modificar."
            )
...
class SuspiciousActivity(models.Model):
    ...
    rule_triggered = models.CharField(
        max_length=30, choices=ReglaActividadSospechosa.choices
    )
    severity = models.CharField(
        max_length=10,
        default=SeveridadActividadSospechosa.MEDIUM,
        choices=SeveridadActividadSospechosa.choices,
    )
    details = models.JSONField(
        help_text="Pruebas recolectadas (IPs, IDs de apuestas, marcas de tiempo)"
    )
    status = models.CharField(
        max_length=15,
        choices=EstadoActividadSospechosa.choices,
        default=EstadoActividadSospechosa.PENDING,
    )
