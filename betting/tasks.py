...
def reactivar_mercados_evento(evento_id):
    ...
    try:
        ...
        if evento.estado in [EstadoEvento.FINALIZADO, EstadoEvento.ANULADO]:
            print(f"Evento {evento_id} ya finalizado/anulado. No se reactivan mercados.")
            return
    ...
    if channel_layer:
        ...
        cuotas_data = {str(s.id): str(s.cuota) for s in selecciones}
...
def actualizar_partidos_a_en_vivo():
    ...
    partidos = Evento.objects.filter(
        estado=EstadoEvento.PROGRAMADO, fecha_inicio__lte=ahora
    )
        ...
        with transaction.atomic():
            ...
            if partido.minuto_actual < 45:
                ...
                if partido.estado != EstadoEvento.FINALIZADO:
                    ...
                    Evento.objects.filter(pk=partido.pk).update(
